...
{
    internal partial class MachineTuneXAxisAcceleration : BaseWizardPage
    {
        ...
        public MachineTuneXAxisAcceleration()
        {
            InitializeComponent();
            lblMachineRunning.Visible = false;
        }

        public MachineTuneXAxisAcceleration(TuningWizardSettings wizardSettings)
            : this()
        {
        ...
        {
            lblAxisHeader.Text = String.Format(GSend.Language.Resources.TuneWizardXAxis, GSend.Language.Resources.TuneWizardAcceleration.ToLower());
            lblTuneAxis.Text = String.Format(GSend.Language.Resources.TuneWizardDescription, "X", GSend.Language.Resources.TuneWizardAcceleration.ToLower());
        ...
        public override bool NextClicked()
        {
            if (!TestInProgress)
            {
                _wizardSettings.NewMaxAccelerationX = _lastGoodValue;
            }
            ...
            try
            {
                using (MouseControl mc = MouseControl.ShowWaitCursor(MainWizardForm))
                {
                    ...
                    string newValueMessage = String.Format(Constants.MessageMachineUpdateSetting,
                        ...
                        $"$120={_wizardSettings.NewMaxAccelerationX}");
                    _wizardSettings.SenderPluginHost.SendMessage(newValueMessage);

                    double currentPos = _wizardSettings.CurrentX;
                    _wizardSettings.SenderPluginHost.SendMessage(String.Format(Constants.MessageMachineWriteLine, _wizardSettings.Machine.Id, $"G1X{currentPos + 50}F20000"));
                    _wizardSettings.SenderPluginHost.SendMessage(String.Format(Constants.MessageMachineWriteLine, _wizardSettings.Machine.Id, $"G1X{currentPos - 100}F20000"));
                    _wizardSettings.SenderPluginHost.SendMessage(String.Format(Constants.MessageMachineWriteLine, _wizardSettings.Machine.Id, $"G1X{currentPos}F20000"));
                ...
                {
                    _lastGoodValue = _wizardSettings.NewMaxAccelerationX;
